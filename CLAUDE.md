...
- **`safe-git-ops`**: Use this for general Git work, especially when an agent might confuse sandbox failures with merge conflicts or when shared worktree metadata makes write paths non-obvious. For routine commit/push closeout, choose the fastest safe path: Fast Path for small text/docs, Skill Path for installed skill behavior, Code Path for helper scripts/tests, and Risk Path for branch/worktree/destructive/release operations.
