...
For ML research projects that use `research-project-memory`, agents must first detect their working scope (`git rev-parse --git-common-dir` vs `--show-toplevel`). If inside a **code-worktree**, read the worktree's `.agent/worktree-status.md` first, then `<ProjectRoot>/memory/BRIEFING.md`, `<ProjectRoot>/memory/project-conventions.md`, and `<ProjectRoot>/memory/hot-results.md`. For Python work, use the shared code uv env by default: `UV_PROJECT_ENVIRONMENT=<ProjectRoot>/.uv-envs/code`; run through `uv run` from the active worktree, and create a stage/worktree env only for dependency or stack changes with the reason recorded. Write in-progress experiment results to the worktree's `.agent/` only; graduate to `ProjectRoot/memory/` only when a result is confirmed and directly changes a project-level claim. This two-level separation prevents in-progress worktree state from polluting the project-wide memory.
...
| `ml-research-router` | Route any ML research task to the correct domain router or high-signal cross-cutting skill. Use when the domain is ambiguous or when the task maps to feedback-synthesizer, advisor-update-writer, rebuttal-strategist, slides, or model/code release. Not for tasks already classifiable into a single domain — prefer the domain router directly |
| `project-ops-router` | Route project operations tasks — git, memory, remote, workspace, code review, timeline, ops — to the correct skill. Use instead of guessing between safe-git-ops, remote-project-control, research-project-memory, new-workspace, or sidecar-task-runner |
| `discovery-router` | Route research discovery tasks — idea validation, literature review, reference reading, corpus comparison, project synthesis — to the correct skill. Use instead of guessing between reference-reading-summarizer, reference-corpus-analyzer, reference-project-synthesizer, or literature-review-sprint |
...
- **`ml-research-router`**: Use as the single entry point when the domain (experiment vs writing vs discovery vs ops) is ambiguous, or when the task clearly maps to a cross-cutting skill like `feedback-synthesizer`, `advisor-update-writer`, `rebuttal-strategist`, `research-slide-deck-builder`, `model-card-writer`, `release-code`, or `artifact-evaluation-prep`. Two-level routing: ml-research-router → domain router → leaf skill. For clear-domain tasks, agents should skip the root router and go directly to the domain router. This avoids unnecessary hops when the domain is obvious.
- **`experiment-evidence-router` / `paper-writing-router` / `discovery-router` / `project-ops-router`**: The four domain routers cover the major ML research workflow domains. Each loads `references/route-table.md` and `references/contrastive-routing.md`. Invoke the relevant domain router when the domain is clear but the specific leaf skill is uncertain.
- **`research-project-memory`**: Use as the project-level coordination layer. It maintains memory boards for claim lifecycle, evidence provenance, source visibility, risks, actions, handoffs, phase dashboard, decisions, components, and worktrees so experiment, writing, review, and rebuttal feedback loops stay aligned. Two compact mandatory-read files anchor cross-session and in-session reliability: `memory/BRIEFING.md` (≤30-line project state snapshot, regenerated at every session closeout) and `memory/hot-results.md` (≤20-line top experiment results digest, updated whenever a key result arrives). Every agent session on a research project must read these two files before any experiment or writing decision. A third mandatory file, `memory/project-conventions.md`, registers the active project-specific rules agents must follow (wrappers, server paths, scope constraints, branch policies) with explicit Active/Suspended/Expired lifecycle so conventions evolve as the project moves through phases rather than accumulating stale instructions.
...
- **`reference-corpus-analyzer`**: Use when 5+ source cards exist and a multi-paper comparison matrix is needed. It assigns tiered read depth (deep for top-N closest, skim for the rest), produces a comparison table across method/task/benchmark/claim dimensions, ranks closest work with one-sentence differentiators, and identifies specific open gaps. Use before `related-work-positioning-writer` and `baseline-selection-audit`.
...
- **`research-results-auditor`**: Use after results are final and before claims are written into the paper. It produces a structured validity verdict covering confounds, claim-drift, protocol integrity, and inferential quality. Slot between `statistical-analysis-planner` and `paper-evidence-board` in the experiment → writing handoff.
...
- **`auto-paper-improvement-loop`**: Use when a draft needs iterative writing quality passes beyond a single consistency edit. Each review round runs in a fresh agent context (no carry-over) to preserve reviewer independence. Declare an edit-whitelist before starting to freeze theorems, numerics, and citations during prose passes. State is checkpointed after each round for crash recovery.
...
- **`model-card-writer`**: Use when releasing a model or dataset publicly, or when a venue requires a model card, datasheet, or reproducibility statement. It generates `MODEL_CARD.md`, `DATASHEET.md`, and short reproducibility statements. Pair with `release-code` before public release and `appendix-organizer` to summarize in the paper appendix.
