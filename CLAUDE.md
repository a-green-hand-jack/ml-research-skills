...
| `submit-paper` | Pre-submission checklist for LaTeX papers: submission mode, mandatory sections, artifacts, anonymity, bibliography, source formatting, Overleaf/GitHub compile handoff by default |
...
- **`paper-writing-memory-manager`**: Use as the global writing-state layer for nonlinear drafting: section status, dependencies, stale prose, style/terminology decisions, edit impact, and open writing threads.
...
- **`init-latex-project`**: Invoked by `project-init` via `scripts/init.sh`. Always expand `~` to the actual home path when calling the script. For paper repos, default to local static checks, Git diff review, commit/push when requested, and Overleaf/GitHub compilation; do not run local LaTeX compile or TeX package-install commands unless the user explicitly asks and a compiler exists.
