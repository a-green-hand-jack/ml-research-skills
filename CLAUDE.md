...
| `submit-paper` | Pre-submission checklist for LaTeX papers: submission mode, mandatory sections, artifacts, anonymity, bibliography, source formatting, and compile-backend handoff |
...
- **`sidecar-task-runner`**: Use for bounded scans, drafts, pre-reviews, milestone proposals, and other low/medium-risk helper work. The main agent writes prompt artifacts under `.agent/sidecars/<task-id>/`, runs one-shot Codex sidecars such as `codex exec --ephemeral -m gpt-5.3-codex-spark`, then verifies or rejects the output before acting.
