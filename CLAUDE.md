...
| `submit-paper` | Pre-submission checklist for LaTeX papers: submission mode, mandatory sections, artifacts, anonymity, bibliography, source formatting, optional compilation |
...
- **`code-reviewer`**: Use after core algorithm or production-code implementation to create `.agent/code-reviews/<change-id>/` bundles and review them from a fresh context. The reviewer should not inherit the writer's chat context and should write findings to `review.md`.
