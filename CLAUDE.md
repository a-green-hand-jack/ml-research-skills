...
| `project-init` | Initialize a research project control root with independent paper/code/slides repos, shared memory, root project docs, root agent guidance, and code/paper worktree policy |
...
- **`project-init` + `project-sync`**: Core paired workflow. `project-init` creates the project control root with independent component repos, shared memory, root project docs, root `AGENTS.md`, and code/paper worktree policies; `project-sync` promotes verified code-side evidence into the paper log.
